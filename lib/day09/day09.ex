defmodule Day09.Day09 do
  ...
  def part_1() do
    ...
    compact(items, Enum.reverse(items), [])
  ...
  defp compact([], _, compacted), do: Enum.reverse(compacted)

  defp compact([block | rest], reversed, compacted) do
    if block == "." do
      {block2, reversed_rest} = get_from_back(reversed)
      compact(rest, reversed_rest, [block2 | compacted])
    else
      compact(rest, reversed, [block | compacted])
    end
  end
